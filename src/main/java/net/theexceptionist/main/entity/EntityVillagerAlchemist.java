...
import net.theexceptionist.main.entity.ai.EntityAIHealAllies;
...
public class EntityVillagerAlchemist extends EntityVillagerSoldier implements IRangedAttackMob{
    ...
    public EntityVillagerAlchemist(World worldIn)
    {
        ...
        this.title  = "Alchemist - "+this.getRandomName();
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor)
    {
        if (!this.isDrinkingPotion())
        {
           ...
           if (f >= 8.0F && !target.isPotionActive(MobEffects.SLOWNESS))
            ...
            else if ((target instanceof EntityZombie || target instanceof EntitySkeleton )  && f <= 3.0F && !target.isPotionActive(MobEffects.WEAKNESS) && this.rand.nextFloat() < 0.25F)
            {
                potiontype = PotionTypes.HEALING;
