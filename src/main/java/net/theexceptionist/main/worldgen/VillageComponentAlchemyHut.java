...
public class VillageComponentAlchemyHut extends StructureVillagePieces.Village
    {
        ...
        public VillageComponentAlchemyHut()
        {
        }

        public VillageComponentAlchemyHut(StructureVillagePieces.Start start, int p_i45566_2_, Random rand, StructureBoundingBox p_i45566_4_, EnumFacing facing)
        {
        ...
        public static VillageComponentAlchemyHut createPiece(StructureVillagePieces.Start start, List<StructureComponent> p_175858_1_, Random rand, int p_175858_3_, int p_175858_4_, int p_175858_5_, EnumFacing facing, int p_175858_7_)
        {
            StructureBoundingBox structureboundingbox = StructureBoundingBox.getComponentToAddBoundingBox(p_175858_3_, p_175858_4_, p_175858_5_, 0, 0, 0, 7, 7, 8, facing);
            return StructureComponent.findIntersecting(p_175858_1_, structureboundingbox) != null ? null : new VillageComponentAlchemyHut(start, p_175858_7_, rand, structureboundingbox, facing);
        }
        ...
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn)
        {
           ...
           // this.fillWithBlocks(worldIn, structureBoundingBoxIn, 1, 0, 1, 5, 0, 5,iblockstate1, iblockstate1, false);
            ...
            this.setBlockState(worldIn,iblockstate4, 6, 0, 7, structureBoundingBoxIn);
            this.setBlockState(worldIn,iblockstate4, 6, 1, 7, structureBoundingBoxIn);
            ...
            this.setBlockState(worldIn,iblockstate4, 6, 0, 3, structureBoundingBoxIn);
            this.setBlockState(worldIn,iblockstate4, 6, 1, 3, structureBoundingBoxIn);
            ...
            if (this.villagersSpawned < count)
            {
                for (int i = villagersSpawned; i < count; ++i)
                {
                   ...
                   // if(worldIn.rand.nextInt(100) <= 20){
                    	...
                    	entityvillager.setLocationAndAngles((double)j + 0.5D, (double)k + 5D, (double)l + 0.5D, 0.0F, 0.0F);
                   ...
                   // }
                        if(worldIn.rand.nextInt(100) <= 20){
                        	EntityVillagerMerchant entityvillager1 = new EntityVillagerMerchant(worldIn);
                        	entityvillager1.setLocationAndAngles((double)j + 0.5D, (double)k + 5D, (double)l + 0.5D, 0.0F, 0.0F);
                            //entityvillager.setSpawnPoint((double)j + 0.5D, (double)k + 5D, (double)l + 0.5D);
                            entityvillager1.setProfession(null);
                            
                            entityvillager1.finalizeMobSpawn(worldIn.getDifficultyForLocation(new BlockPos(entityvillager)), (IEntityLivingData)null, false);
